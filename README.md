...
This is the strongest skill-engagement mechanism in the stack — not a passive reminder, not an "always cite" suggestion, but a hard gate on the next file edit. The LLM cannot start implementing M3 without first writing, visibly and verifiably, what it considered from `elixir-planning`, `elixir-implementing`, `elixir-reviewing`, `phoenix`, and `phoenix-liveview` for that milestone. The plan and the skills are pulled into a single scan-able artifact that proves "the right knowledge was on the table when the work began."
